...
pub(crate) const MODE: &str = "qualify-gpu-native-source-mapped-pin-production";
pub(crate) const SCHEMA: &str = "mer.gpu-native-source-mapped-pin-production.v1";
pub(crate) const BEGIN: &str =
    "HMA1FB_QUALIFIER_BEGIN order=mapped-baseline,mapped-pinned,mapped-pinned,mapped-baseline";
pub(crate) const END: &str = "HMA1FB_QUALIFIER_COMPLETE payload_sha256=";
const PRIMARY: &str = "Measured K>1 only. Pinned minus baseline; percentage denominator is the corresponding baseline integer total. Width 1 and batch_sum_read_wall_ns are descriptive only.";
const CLEANUP: &str = "Exact per-source pre/active/after VmPin and VmLck accounting; K*FULL pin delta only during unchanged source reads. Explicit unregister and ring drop before view teardown. Exact arm process cleanup; no tolerance.";
...
pub(crate) async fn run_worker(args: CommandArgs) -> Result<()> {
    ...
    let result = async {
        ...
        for (position, mode) in ORDER.into_iter().enumerate() {
            ...
            let run = run_physical_install_arm_observed_inner(
                &prepared,
                &args,
...
pub(crate) fn launch(
    ...
    require(
        ...
        "HMA-1F-B outputs must be fresh",
    )?;
    ...
    eprintln!(
        "HMA1FB_RESULT={} raw_report_sha256={} transcript_sha256={}",
    ...
    require(
        ...
        "HMA-1F-B run non-authoritative; see raw report/transcript",
    )
}
fn worker_arguments(
    config: &Path,
    ...
    require(
        ...
        "HMA-1F-B forbids autotune/probe workloads",
    )?;
...
impl Comparison {
    fn new(baseline: Endpoints, pinned: Endpoints) -> Self {
        Self {
            baseline,
            pinned,
            dpin_crit: i128::from(pinned.critical) - i128::from(baseline.critical),
            dpin_max: i128::from(pinned.max_wall) - i128::from(baseline.max_wall),
        }
    }
    fn negative(&self) -> bool {
        self.dpin_crit < 0 && self.dpin_max < 0
    }
    fn positive(&self) -> bool {
        self.dpin_crit > 0 && self.dpin_max > 0
    }
    fn at_most(&self, p: i128) -> bool {
        self.dpin_crit * 100 <= i128::from(self.baseline.critical) * p
            && self.dpin_max * 100 <= i128::from(self.baseline.max_wall) * p
    }
...
fn classify(a: &Analysis) -> &'static str {
    ...
    let neg = a.blocks.len() == 2
        ...
        && a.widths
            .iter()
            .filter(|s| s.comparison.dpin_crit < 0)
            .count()
            >= 5;
    let pos = a.blocks.len() == 2
        ...
        && a.widths
            ...
            >= 5;
    if p.at_most(-3) && neg {
        "PIN_MITIGATES_STRAGGLER"
    } else if p.at_most(-1) && !p.at_most(-3) && neg {
        "PIN_DIRECTIONAL_MITIGATION"
    } else if p.dpin_crit * 100 > -i128::from(p.baseline.critical)
        && p.dpin_crit * 100 < i128::from(p.baseline.critical)
        && p.dpin_max * 100 > -i128::from(p.baseline.max_wall)
        && p.dpin_max * 100 < i128::from(p.baseline.max_wall)
    {
        "PIN_NO_MATERIAL_EFFECT"
    } else if p.at_least(3) && pos {
        "PIN_WORSENS_STRAGGLER"
    } else {
        "AMBIGUOUS_PIN_INTERACTION"
    }
}
...
fn validate_transcript(e: &Envelope, transcript: &[u8]) -> Result<()> {
    ...
    require(
        t.lines().filter(|l| *l == BEGIN).count() == 1
            && t.matches("HMA1FB_QUALIFIER_BEGIN").count() == 1
            && t.matches("HMA1FB_QUALIFIER_COMPLETE").count() == 1
            && t.trim_end()
...
fn audit_bytes(raw: &[u8], transcript: &[u8]) -> Audit {
    let mut audit = Audit {
        schema: "mer.gpu-native-source-mapped-pin-production.audit.v1",
...
pub(crate) fn audit_command(raw: &Path, transcript: &Path, report_out: &Path) -> Result<()> {
    ...
    require(
        audit.authoritative,
        "HMA-1F-B NON_AUTHORITATIVE; see audit report",
    )
}
...
mod hma1fb_tests {
    ...
    #[test]
    fn hma1fb_exact_request_and_width_sign_thresholds_both_directions() {
        for (delta, expected) in [
            (-400, "PIN_MITIGATES_STRAGGLER"),
            (400, "PIN_WORSENS_STRAGGLER"),
        ] {
            let mut a = analysis(delta);
            for r in &mut a.request_pairs[..2] {
                r.comparison = analysis(-delta).pooled;
            }
            for r in &mut a.widths[..2] {
                r.comparison = analysis(-delta).pooled;
            }
            assert_eq!(classify(&a), expected); // exactly 4/6 and 5/7
            a.request_pairs[2].comparison = analysis(0).pooled;
            assert_eq!(classify(&a), "AMBIGUOUS_PIN_INTERACTION");
            let mut a = analysis(delta);
            for r in &mut a.widths[..3] {
                r.comparison = analysis(0).pooled;
            }
            assert_eq!(classify(&a), "AMBIGUOUS_PIN_INTERACTION");
        }
    }
    #[test]
    fn hma1fb_integer_products_handle_full_u64_totals_without_rounding() {
        let b = Endpoints {
            critical: u64::MAX,
            max_wall: u64::MAX,
        };
        ...
        assert!(!c.at_most(-1)); // -0.990099%, strictly inside the neutral interval
        let c = Comparison::new(
            Endpoints {
                critical: 101,
                max_wall: 101,
    ...
    #[test]
    fn hma1fb_pin_authority_failure_precedes_performance_classification() {
        for mutate in 0..7 {
            let mut p = fixture();
            let r = &mut p.arms[1].records[0];
            match mutate {
                0 => r.pin.process.active.as_mut().unwrap().vmpin_bytes += 1,
                1 => r.pin.process.after.as_mut().unwrap().vmpin_bytes += 1,
                2 => r.pin.process.active.as_mut().unwrap().vmlck_bytes += 1,
                3 => r.pin.sqes_submitted = 1,
                4 => r.pin.ring_dropped = false,
                5 => r.pin.registration.failures = 1,
                _ => r.failure = Some("source read failed".into()),
            }
            let a = audit(p);
    ...
    fn hma1fb_launch_rejects_before_paths_or_runtime_are_touched() {
        let error = launch(
        ...
        .unwrap_err();
        assert!(error.to_string().contains("Linux + io_uring"));
    }
    use super::*;
    ...
    fn record(mode: Mode, measured: bool, request_index: usize, width: usize, wall: u64) -> Record {
        let ranges = (1..=width)
            ...
            .collect();
        let mut pin = PinEvidence::new(mode, ranges);
        pin.ring.create.attempts = 1;
        pin.ring.create.successes = 1;
        pin.ring_dropped = true;
        let pre = ProcessMemory {
            vmpin_bytes: 4096,
            vmlck_bytes: 8192,
        };
        let mut active = pre.clone();
        if mode == Mode::MappedPinned {
            pin.registration.attempts = 1;
            pin.registration.successes = 1;
            pin.unregister.attempts = 1;
            pin.unregister.successes = 1;
            pin.iovec_count = width;
            pin.requested_pin_bytes = (width * FULL) as u64;
            active.vmpin_bytes += pin.requested_pin_bytes;
        }
        pin.process.pre = Some(pre.clone());
        pin.process.active = Some(active);
        pin.process.after = Some(pre);
        Record {
            mode,
            width,
            measured,
    ...
    #[test]
    fn hma1fb_complete_offline_reconstruction_accepts_four_distinct_contexts() {
        let a = audit(fixture());
        assert!(a.authoritative, "{:?}", a.failure);
        assert_eq!(a.classification, "PIN_NO_MATERIAL_EFFECT");
        assert_eq!(a.analysis.unwrap().request_pairs.len(), 6);
    }
    #[test]
    fn hma1fb_wrong_order_rejects() {
        let mut p = fixture();
        p.arms.swap(0, 1);
        assert!(!audit(p).authoritative);
    }
    #[test]
    fn hma1fb_no_fifth_arm() {
        let mut p = fixture();
        p.arms.push(p.arms[0].clone());
        assert!(!audit(p).authoritative);
    }
    #[test]
    fn hma1fb_runtime_context_reuse_rejects() {
        let mut p = fixture();
    ...
    #[test]
    fn hma1fb_invalid_context_ids_reject() {
        for value in [
            json!("0"),
            json!("01"),
            json!("-1"),
    ...
    #[test]
    fn hma1fb_second_runtime_difference_rejects() {
        let mut p = fixture();
        p.arms[1].production["benchmark"]["runtime_contract"]["extra"] = json!(true);
        assert!(!audit(p).authoritative);
    }
    #[test]
    fn hma1fb_each_semantic_contract_drift_rejects() {
        for field in [
            "request",
            "production_configuration",
            "production_semantics",
    ...
    #[test]
    fn hma1fb_token_text_route_source_width_drift_rejects() {
        for mutation in 0..5 {
            let mut p = fixture();
            match mutation {
                0 => {
    ...
    #[test]
    fn hma1fb_request_pair_position_drift_rejects() {
        let mut p = fixture();
        p.arms[2].records[2].request_index = 0;
        assert!(!audit(p).authoritative);
    }
    #[test]
    fn hma1fb_pin_evidence_corruption_rejects() {
        for field in ["registration", "unregister", "ranges", "process", "ring"] {
            let p = fixture();
            let mut r = serde_json::to_value(&p.arms[1].records[0]).unwrap();
            r["pin"][field] = Value::Null;
            assert!(serde_json::from_value::<Record>(r).is_err());
        }
        let mut p = fixture();
        p.arms[1].records[0].pin.unregister.failures = 1;
        assert!(!audit(p).authoritative);
    }
    #[test]
    fn hma1fb_baseline_syscall_evidence_rejects() {
        let mut p = fixture();
        p.arms[0].records[0].pin.registration.attempts = 1;
        assert!(!audit(p).authoritative);
    }
    #[test]
    fn hma1fb_vmlck_leak_rejects() {
        let mut p = fixture();
        p.arms[1].after.vmlck_bytes = 4096;
        assert!(!audit(p).authoritative);
    }
    #[test]
    fn hma1fb_recovery_and_source_failure_rejects() {
        for (object, key) in [
            ("work", "recovery"),
            ("work", "token_loop"),
    ...
    #[test]
    fn hma1fb_fd_proof_failure_rejects() {
        let mut p = fixture();
    ...
    #[test]
    fn hma1fb_transient_retry_pattern_rejects() {
        contaminated(PATTERNS[0]);
    }
    #[test]
    fn hma1fb_recovered_retry_pattern_rejects() {
        contaminated(PATTERNS[1]);
    }
    #[test]
    fn hma1fb_fetch_retry_pattern_rejects() {
        contaminated(PATTERNS[2]);
    }
    #[test]
    fn hma1fb_breaker_pattern_rejects() {
        contaminated(PATTERNS[3]);
    }
    #[test]
    fn hma1fb_all_patterns_reject() {
        contaminated(&PATTERNS.join("\n"));
    }
    #[test]
    fn hma1fb_transcript_hash_truncation_duplicate_markers_reject() {
    ...
    #[test]
    fn hma1fb_raw_payload_hash_mismatch_rejects() {
        let (r, t) = bound(fixture(), "");
        let mut e: Envelope = serde_json::from_slice(&r).unwrap();
    ...
    #[test]
    fn hma1fb_exact_payload_snapshot_preserves_json_spelling() {
        let (r, t) = bound(fixture(), "");
        let mut e: Envelope = serde_json::from_slice(&r).unwrap();
    ...
    #[test]
    fn hma1fb_cached_analysis_cannot_override_audit() {
        let (r, t) = bound(fixture(), "");
    ...
    #[test]
    fn hma1fb_classifier_exact_integer_thresholds() {
        for (d, label) in [
            (-301, "PIN_MITIGATES_STRAGGLER"),
            (-300, "PIN_MITIGATES_STRAGGLER"),
            (-299, "PIN_DIRECTIONAL_MITIGATION"),
            (-100, "PIN_DIRECTIONAL_MITIGATION"),
            (-99, "PIN_NO_MATERIAL_EFFECT"),
            (0, "PIN_NO_MATERIAL_EFFECT"),
            (99, "PIN_NO_MATERIAL_EFFECT"),
            (100, "AMBIGUOUS_PIN_INTERACTION"),
            (299, "AMBIGUOUS_PIN_INTERACTION"),
            (300, "PIN_WORSENS_STRAGGLER"),
            (301, "PIN_WORSENS_STRAGGLER"),
        ] {
            assert_eq!(classify(&analysis(d)), label, "{d}");
        }
    }
    #[test]
    fn hma1fb_classifier_requires_both_endpoints_and_all_sign_strata() {
        ...
        assert_eq!(classify(&a), "AMBIGUOUS_PIN_INTERACTION");
        let mut a = analysis(-400);
        a.pooled.dpin_max = -50;
        assert_eq!(classify(&a), "AMBIGUOUS_PIN_INTERACTION");
    }
    #[test]
    fn hma1fb_width_one_never_enters_primary() {
        let mut p = fixture();
    ...
    #[test]
    fn hma1fb_block_b_is_pinned_minus_baseline() {
        let mut p = fixture();
        ...
        let a = analyze(&p.arms).unwrap();
        assert_eq!(a.blocks[1].comparison.dpin_crit, -30);
        assert_eq!(a.request_pairs[3].comparison.dpin_crit, -10);
    }
    #[test]
    fn hma1fb_launcher_preserves_process_arguments_on_both_sides_of_command() {
        use clap::Parser;
        for global_before in [false, true] {
            ...
            assert!(
                matches!(cli.cmd, crate::Cmd::Hma1fbWorkerInternal { config, report_out } if config == Path::new(FROZEN_CONFIG_PATH) && report_out == Path::new("raw.payload.json"))
            );
        }
    }
    #[test]
    fn hma1fb_auditor_and_probe_dispatch_before_runtime_setup() {
        let s = include_str!("main.rs");
        let main = s.split("fn main() ->").nth(1).unwrap();
        for marker in ["mapped_pin::audit_command", "mapped_pin::launch"] {
