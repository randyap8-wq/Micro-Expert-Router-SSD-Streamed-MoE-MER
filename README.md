...
| `inference` | SwiGLU expert FFN (`y = down · (silu(gate·x) ⊙ (up·x))`). Homogeneous Q8_0 experts use a native zero-copy block-stream kernel over `ExpertResident::data()`: scalar on portable hosts, AVX2+FMA on x86_64, and AVX-512F+BW when built with `avx512` and supported at runtime. The SIMD path keeps row-wide accumulators and fuses gate/up activation loads. Candle's copying Q8_0 `QStorage` path is retained only behind `q8-candle-reference`. Other dtypes retain their existing Candle/dequant/QMatMul execution paths, and UTH2 mixed experts dispatch each projection from its own validated byte range. The storage, routing, prefetch, and model architecture remain unchanged. |
...
##### Admission control, API keys & rate limiting
...
| ------------------------------------- | ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `[server].max_concurrent_requests`    | `0` (off) | Hard ceiling on in-flight HTTP requests. Excess requests are rejected with `503 Service Unavailable` so the engine never silently queues unboundedly under load.                                                                                                                          |
| `[server].admission_min_free_blocks`  | `0` (off) | Paged-KV watermark. When a `[real_transformer]` block pool is configured and free blocks would drop below this number, new requests are rejected with `503` before they can consume the last reserves the in-flight requests need to finish.                                              |
...
#### Native Q8_0 expert microbenchmark
...
micro-expert-router gguf-convert
  --gguf-path <PATH>         Source GGUF checkpoint, or any shard in a
                              standard *-00001-of-00005.gguf set
...
its own dtype. Block-aligned Q4_0/Q4_K/Q5_K/Q6_K/Q8_0 projections run
through Candle `QMatMul` over the packed block stream; `QStorage::from_data`
temporarily copies one packed projection and drops that copy after the
projection call. Unsupported or misaligned quantized shapes use the
counted scalar reference fallback, while full-matrix dequantization is
tracked separately. Normal mixed execution should report optimized
projection counters and leave `scalar_fallbacks` and
`full_dequant_fallbacks` at zero.
